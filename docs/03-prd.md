...
- **Maps/routing are non-Google:** map render + segment overlays = **MapLibre GL + OpenFreeMap** (keyless); route check/recommendation = **OpenRouteService (ORS)** (F-001, F-003, F-005). The map/routing stack is owned by [System Design](./06-system-design.md).
...
| F-005 | Severity-tiered route recommendation — **2 routes max** (safest + 1 alternative) | P0 | "One route recommendation doesn't show the tradeoff between safety and directness" | `frontend/src/lib/routing.js` `fetchSafeRoutes`; red hard-avoid, yellow soft-avoid, green informational only; **cap at 2** (safest computed path + 1 visible alternative; 1 route only when no distinct alternative exists) — decision: [mentor-synthesis §4](./analysis/mentor-synthesis.md), logged in [POSTMORTEM §3](./POSTMORTEM.md); see `docs/superpowers/specs/2026-07-01-severity-tiered-ai-routing-design.md` |
...
- **F-005:** Given a destination, the system returns **at most 2** route candidates (the safest
  computed path plus one alternative; 1 only when no distinct alternative exists), safest first;
...
- **OpenRouteService (ORS)** — route lookup / recommendation (F-003, F-005). Client-side key, not yet origin-restricted `[unverified]`.
